...
#### 语言设置
#### 主题样式
- 关于应用：最上方是LOGO图标（居中），下方是应用中文名：，应为介绍：本应用
- 更新日志：显示历次的更新日志简介
...
##### 点击默认的阅读界面中部（屏幕中间1/3区域），同时弹出顶部工具栏和底部工具栏（这个底部工具栏与默认阅读界面布局的底部工具栏不同）

##### 顶部工具栏
- 顶部工具栏从左到右，显示左箭头（返回），章节名称， 顶部工具栏的最右边显示三个点图标。 点击三个点图标，弹出菜单具体内容待定。其它图标迁移到底部工具栏（后续再确定）。

##### 底部工具栏
- 底部工具栏分为上下两行。
- 底部工具栏第1行，左边占比2/3宽度显示：左箭头（点击可以翻页到上一页），滑杆（中间有个圆点可以左右拖动，控制翻页进度），右箭头（点击可以翻页到下一页）。右边1/3宽度显示页码（例如：2/13）和阅读百分比（例如：68.1%）。
- 底部工具栏第2行，放置各种操作图标，从左到右依次是：目录图标（点击可以弹出目录链接）、朗读图标（具体功能描述后续再提出）、暂停朗读图标、停止朗读图标、合成语音图标（具体功能后续再实现）、语音列表图标（具体功能后续再实现）、分享图标、阅读设置图标（可设置字体大小、行间距等）


##### 阅读页面的顶部工具栏，继续完善：
- 点击三个点图标，弹出菜单包括：查看目录、从本页朗读、合成语音、语音列表、分享内容、阅读设置

##### 底部工具栏，做一下调整：
- 底部工具栏分为上下两行。
- 底部工具栏第1行：取消显示页码和阅读百分比。整行都用来显示：左箭头（点击可以翻页到上一页），滑杆（中间有个圆点可以左右拖动，控制翻页进度），右箭头（点击可以翻页到下一页）。拉取滑杆中间的小圆点，圆点滑动位置需与 页码/所有页码 的比例一致！
- 底部工具栏第2行，放置各种操作图标，从左到右依次是：目录图标（点击可以弹出目录链接）、朗读图标（具体功能描述后续再提出）、合成语音图标（做成录音的图标）、语音列表图标（具体功能后续再实现）、分享图标、阅读设置图标（可设置字体大小、行间距等）。

请对TTS朗读功能进行完善，具体需求如下：
##### TTS朗读：
