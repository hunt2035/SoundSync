...
### **书库**
- 点击主页的底部导航栏的书库图标，进入书库界面，一级页面列出以下几个分类名称：新建文本书籍、网址导入书籍、本地导入书籍、合成语音文件。分类名称左边有一个合适的图标，分类名称右边是文件的数量、文件浏览器图标。点在文件浏览器图标，可以打开系统自带的浏览器，自动定位到对应的目录。
...
#### 全局TTS状态与UI的关系
- 朗读时，当前朗读的句子需高亮显示，且朗读的句子与高亮的句子需要保持同步。不论TTS朗读、还是在文字高亮的组件接收句子列表，都是按照整个页面的句子序列进行。即不用采用段落划分机制，只采用两个级别划分：1）全书的所有文字分为多个页面文字； 2）每个页面按照AppTextUtils的SENTENCE_DELIMITER_REGEX规则拆分的句子序列，不需要采用段落划分文字。

- **音频播放控件**：两边是半圆形，从左到右分别是圆形的封面，"边听边看"文字按钮，播放按钮（▶）/暂停按钮（⏸）、停止按钮（❌）、三个短横线小图标。
...
3. 如果当前阅读的位置的readBookId和readCurrentPage 与 当前朗读的位置的currentBookId和 currentPageIndex相同，则:
    - 当全局TTS状态等于“朗读状态”或“暂停状态”时，**音频播放控件**里的文字按钮从“边听边看”改成“同步朗读中”，文字的背景改为浅蓝色，文字按钮不可点击。同时，TTS朗读或暂停时的文字高亮功能起作用。
    - 当全局TTS状态等于“停止状态”时，**音频播放控件**隐藏。同时，不产生TTS朗读的文字高亮。
4. 如果当前阅读的位置的readBookId 与 当前朗读的位置的currentBookId不一致，或者如果当前阅读的位置的readCurrentPage与 currentPageIndex不同，或者用户切换到非阅读界面（例如书架等页面），则满足以下规则:
    - 当全局TTS状态等于朗读状态或暂停状态时，**音频播放控件**里的文字按钮变成“边听边看”，文字的背景改为深蓝色。同时，TTS朗读或暂停时的页面文字不进行高亮显示。 如果用户点击“边听边看”，则页面立即切换到当前朗读的位置(currentBookId和 currentPageIndex)对应的页面。
    - 当全局TTS状态等于“停止状态”时，**音频播放控件**隐藏。同时，页面的文字不进行高亮显示。
...
在早期版本中，当页面包含多个段落时，TTS朗读高亮显示会出现不准确的情况，表现为多个句子同时高亮。这是因为每个段落被单独传递给HighlightedText组件，而TTS朗读是按照整个页面的句子序列进行的。

在版本1.0.2中，我们修复了这个问题：

1. 修改了句子处理机制，取消了段落划分，直接使用整个页面的句子序列
2. 在页面加载时，通过`setPageSentences`方法设置整个页面的句子列表
3. 确保TtsManager和HighlightedText组件使用相同的句子序列
4. 优化了高亮显示逻辑，确保只有当前朗读的句子被高亮显示

这些改进确保了TTS朗读与高亮显示的完全同步，提供了更好的阅读体验。
