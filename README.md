...
4. 如果当前阅读的位置的readBookId和readCurrentPage 与 当前朗读的位置的currentBookId和 currentPageIndex不同（只要任意一个变量不同），或者用户切换到非阅读界面（例如书架等页面），则:
    - 当全局TTS状态等于朗读状态或暂停状态时，**音频播放控件**里的文字按钮变成“边听边看”，文字的背景改为深蓝色。同时，TTS朗读或暂停时的文字高亮功能不起作用。 如果用户点击“边听边看”，则页面立即切换到当前朗读的位置(currentBookId和 currentPageIndex)对应的页面。
    - 当全局TTS状态等于“停止状态”时，**音频播放控件**隐藏。同时，不产生TTS朗读的文字高亮。
