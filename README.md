...
### v0.22 (2024-XX-XX)
- **功能优化**: 移除不必要的录音权限和相关功能
- **简化应用**: 专注于TTS语音播放和合成功能
- **权限精简**: 减少应用所需的权限，提高用户隐私保护

### v0.21 (2024-XX-XX)
- **新增功能**: 将新建文本文件也存储到外部Documents目录
...
##### TTS语音合成：
- 用户点击语音合成图标，弹出以下界面，默认合成当前章节(如果是TTS，当前章节就是TTS文件全文)，然后点开始合成，合成语音操作开始在后台进行，在语音合成和文件保存的过程中，要及时向用户反馈操作的进展情况，比如显示语音合成的进度条、文件保存成功或失败的提示信息等，让用户清楚了解当前操作的状态，避免用户因长时间等待无反馈而产生焦虑情绪或误操作。即使用户关闭窗口再点击进来，也可以看到合成的进度情况。
┌────────────────┐
│      语音合成                  │
├────────────────┤
│ 合成范围:                      │
