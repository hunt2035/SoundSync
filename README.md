...
- 书架：是默认主页面，包括顶部导航栏、书籍列表、底部导航栏。顶部导航栏左边显示"我的书架"，右边显示搜索图标和三个点的菜单按钮图标，点击后弹出菜单包括新建文本、网址导入、本地导入、拍照导入、书架管理、书籍排序。书籍列表默认按照添加日期（addedDate）倒序排列。
- 书库：访问公共目录的书籍，待开发
...
##### PDF文件处理
- 导入PDF文件时，系统会自动提取PDF中的文本内容，并保存为TXT文件
- 保存的TXT文件与原PDF文件同名且位于相同目录
- 系统会记录原始PDF文件路径，确保文件溯源关系完整
- 用户可以像阅读TXT文件一样阅读提取出的文本内容，并支持TTS朗读功能
- 如果PDF文本提取失败或提取结果为空，系统将使用原始PDF文件进行导入
...
- 对于合成语音文件，可以进行设置闹钟操作。
  ...
  - 设置闹钟，点确定时，将该语音文件复制到Music/ringtone目录下（如果没有ringtone目录则自动创建），然后立即刷新媒体库（比如用MediaScannerConnection.scanFile），传入设置闹钟语音文件在ringtone目录的绝对路径，使系统可以识别到新音频文件。
...
1. 最上方是LOGO图标（居中）
2. 应用中文名：漫阅
3. 应用英文名和版本号：Wander Reads v+版本号
...
- 点击一个语音文件，下方弹出播放工具栏，从左到右为：倒退图标（点击后可倒退语音15秒）、播放▶或暂停⏸按钮（如播放时点击后则语音暂停，如语音停止时点击则语音继续播放）、快进图标（点击后可前进语音15秒）、重命名图标、删除图标、三个点菜单。
