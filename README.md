...
- TTS的状态共三种，1）停止状态；2）朗读状态；3）暂停状态。app需要记录TTS的状态，对于朗读状态和暂停状态，还需要记录当前朗读的页码，以便随时可以跳转到朗读的页面。app可以根据TTS的状态确定UI及按钮的状态。
...
- 阅读界面底部工具栏的文字按钮“朗读本页”，意思是从本页开始朗读。如果TTS状态为朗读状态，则“朗读本页”置灰，不可点击，否则，用户点击“朗读本页”后，开始从本页开始朗读，TTS状态变成朗读状态。

- 悬浮控件支持被拖动到在整个阅读界面的任意位置。
- 只有用户点击音频播放控件的暂停/播放或停止按钮，或完成播放，或播放异常中断，才会影响朗读状态，需同步更新UI。
- 音频播放控件包含三个控制按钮：播放（▶）、暂停（⏸）、停止（⏹）。播放按钮和暂停按钮是在一个图标位置，默认是播放按钮，用户点击后切换到暂停按钮，再点击又回到播放按钮。如用户点停止按钮则停止播放，同时音频播放控件立即隐藏。
