...
##### 阅读界面整体变化：
- 点击默认的阅读界面中间区域（屏幕中间1/3区域），同时弹出顶部工具栏、底部工具栏，以及朗读控制悬浮控件（如有语音播放）。此时，点击阅读内容，不再响应翻页的指令，而是无论点击阅读内容的任何一个区域，都是让顶部工具栏、底部工具栏以及朗读控制悬浮控件同时消失。
...
- 全局TTS状态共三种，1）停止状态（STATUS_STOPPED = 0）；2）朗读状态（STATUS_PLAYING = 1）；3）暂停状态（STATUS_PAUSED = 2）。app需要记录TTS的状态，对于朗读状态和暂停状态，还需要记录当前朗读的位置（包括书籍ID、书籍页码、朗读的文字），以便随时可以跳转到朗读的书籍页面。app可以根据TTS的状态确定UI及按钮的状态。
...
- 在默认阅读界面（即无顶部工具栏和底部工具栏时），音频播放控件隐藏。
- 在阅读界面有顶部工具栏和底部工具栏时，app判断TTS的状态：如果是停止状态，则音频播放控件隐藏；如果TTS是朗读状态，则音频播放控件可见，且显示暂停按钮（⏸）；如果TTS是暂停状态，则音频播放控件可见，且显示播放按钮（▶）。

-. 底部工具栏上，用户点击文字按钮“朗读本页”后，开始朗读，全局TTS状态变为朗读状态，此时，文字按钮颜色变灰色，不可点击，同时文字变成“朗读中...”。目前文字内容是空白，是不对的。 
-. 当TTS状态变成暂停状态时，底部的朗读按钮又恢复成“朗读本页”，且可以点击了，用户点击后，首先自动关闭之前的朗读，然后重新朗读本页，TTS全局状态又变更为朗读状态，且朗读的位置更新为从本书的本页开始。
-. 当TTS状态变成停止状态时，底部的朗读按钮又恢复成“朗读本页”，且可以点击了，用户点击后，又重新朗读本页，TTS全局状态又变更为朗读状态，且朗读的位置更新为从本书的本页开始。
